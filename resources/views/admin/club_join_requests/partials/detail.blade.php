@php
    ...
    $canSchedule = in_array($status, ['pending', 'scheduling_interview', 'interview']) && !in_array($status, ['approved', 'rejected', 'cancelled']);
    $canCompleteInterview = in_array($status, ['interview']) && !in_array($status, ['approved', 'rejected', 'cancelled']);
    $canDecide = in_array($status, ['pending', 'interview_completed']) && !in_array($status, ['approved', 'rejected', 'cancelled']);
    $canCancel = in_array($status, ['pending', 'scheduling_interview', 'interview', 'interview_completed']) && !in_array($status, ['approved', 'rejected', 'cancelled']);
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <p class="mb-0"><strong>MSSV:</strong> {{ $request->user->member->student_code ?? '—' }}</p>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.club_join_requests.handle', $request->id) }}"
                            ...
                            <div class="mb-2">
                                ...
                                <select name="interviewer_id" class="form-select form-select-sm" required>
                        ...
                        <form method="POST" action="{{ route('admin.club_join_requests.handle', $request->id) }}"
                            ...
                            <div class="mb-2">
                                ...
                                <select name="interview_result" class="form-select form-select-sm" required>
                                    <option value="completed" @selected($request->interview_result === 'completed')>Hoàn thành
                                    </option>
