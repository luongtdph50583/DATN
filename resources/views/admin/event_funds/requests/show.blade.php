        ...
        <div class="card-body">
            <table class="table table-bordered mb-0">
                    ...
                    <td>
                        @switch($request->status)
                            @case('pending_disbursement')
                                ...
                                @php $person = $request->disbursedBy ?? null; @endphp
                                @break
                            @case('disbursing')
                            ...
                            @case('rejected')
                                ...
                                @php $person = $request->rejectedBy ?? null; @endphp
                                @break
                            @default
                ...
                <tr>
                    ...
                    <td>
                        @if($request->status === 'disbursed' || $request->status === 'disbursing' || $request->status === 'pending_disbursement')
                            {{ $request->disbursement_date ? \Carbon\Carbon::parse($request->disbursement_date)->format('d/m/Y H:i') : '-' }}
                        @elseif($request->status === 'rejected')
                            {{ $request->updated_at ? $request->updated_at->format('d/m/Y H:i') : '-' }}
                        @else
                            -
                        @endif
                    </td>
                ...
                <tr>
                    <th>Minh chứng / Lý do</th>
                    <td>
    @if($request->disbursement_proof)
        @php
            $files = json_decode($request->disbursement_proof, true);
        @endphp
        @foreach($files as $file)
            <a href="{{ asset('storage/' . $file) }}" target="_blank">Xem minh chứng {{ $loop->iteration }}</a><br>
        @endforeach
    @else
        -
    @endif
