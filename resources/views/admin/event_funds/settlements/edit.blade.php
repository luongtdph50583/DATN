...
<div class="container">
    <form action="{{ route('admin.event_fund_settlements.update', $settlement->id) }}" method="POST" enctype="multipart/form-data">
        ...
        @if(count($currentReceipts) > 0)
            <div class="mb-4">
                <label class="form-label">Hóa đơn hiện tại (click [x] để xóa)</label>
                <div class="receipt-grid">
                    @foreach($currentReceipts as $index => $filePath)
                        @php
                            $path = is_string($filePath) ? $filePath : ($filePath['path'] ?? null);
                            if (!$path) continue;
                        ...
                        <div class="receipt-item" data-path="{{ $path }}">
                            ...
                            <!-- NÚT XÓA -->
                            <button type="button" class="remove-receipt" data-path="{{ $path }}">
                                <i class="fas fa-times"></i>
                            </button>

                            <!-- Input giữ lại -->
                            <input type="hidden" name="keep_receipts[]" value="{{ $path }}" class="keep-input">
                        </div>
                    @endforeach
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.remove-receipt').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const input = item.querySelector('.keep-input');
            if (!input) return;
