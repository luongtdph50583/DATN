                        ...
                        <div class="col-md-6">
                            ...
                            @if($log->type === 'proposer' && $log->proposer)
                                <tr>
                                    <th>Người đề xuất:</th>
                                    <td>
                                        <div><strong>{{ $log->proposer->name ?? 'Không rõ' }}</strong></div>
                                        <div>Email: {{ $log->proposer->email ?? '—' }}</div>
                                        @if(isset($log->proposer->memberInfo))
                                            <div>MSSV: {{ $log->proposer->memberInfo->student_code ?? '—' }}</div>
                                            <div>Điện thoại: {{ $log->proposer->memberInfo->phone ?? '—' }}</div>
                                            <div>Khóa học: {{ $log->proposer->memberInfo->course ?? '—' }}</div>
                                            <div>Ngành học: {{ $log->proposer->memberInfo->major ?? '—' }}</div>
                                        @endif
                                    </td>
                                </tr>
                            @endif
                                    ...
                                    <td>
                                        @if($log->admin)
                                            <div class="d-flex align-items-center">
                                                @if($log->admin->avatar)
                                                    <img src="{{ $log->admin->avatar }}" alt="Avatar" class="rounded-circle me-2"
                                                        style="width:40px;height:40px;object-fit:cover;">
                                                @endif
                                                <div>
                                                    <div><strong>{{ $log->admin->name }}</strong></div>
                                                    <div>Email: {{ $log->admin->email ?? '—' }}</div>
