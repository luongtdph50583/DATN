...
@section('card-body')
        ...
        <div class="card-body">
            <table class="table table-borderless">
                ...
                <tr>
                    ...
                    <td>
                        {{-- Ví dụ link tới bài viết --}}
                        <a href="{{ route('admin.posts.show', $log->post_id) }}" class="btn btn-sm btn-primary"
                            target="_blank">
                            <i class="fas fa-eye"></i> Xem bài viết
                        </a>
                        {{-- Nếu muốn link tới CLB thì thêm --}}
                        <a href="{{ route('admin.clubs.show', $log->post->club_id) }}"
                            class="btn btn-sm btn-outline-secondary ms-2" target="_blank">
                            <i class="fas fa-users"></i> Xem CLB
                        </a>
                    </td>
                </tr>
            </table>
    ...
    function displayValue($val, $field)
    {
        ...
        if ($field === 'visibility')
            return $val === 'internal' ? 'Nội bộ' : 'Công khai';
        if ($field === 'type') {
            return match ($val) {
                'post' => 'Bài viết',
                'notice' => 'Thông báo',
                ...
                <table class="change-table">
                    ...
                    <tbody>
                        @foreach($changes as $field => $vals)
                            @if($field !== 'content')
                                <tr>
                                    <td class="fw-bold text-primary">{{ fieldLabel($field) }}</td>
                                    <td>
        ...
        @if(isset($changes['content']) && is_array($changes['content']))
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-light">
                    <h5 class="mb-0"><i class="fas fa-file-alt me-2"></i>Nội dung</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
