...
<script>
$oldReceipts = json_decode($fund->receipt, true) ?? [];
$keepReceipts = $request->input('keep_receipts', []);

// Xóa file không tick checkbox
foreach(array_diff($oldReceipts, $keepReceipts) as $file){
    Storage::disk('public')->delete($file);
}

// Cập nhật DB
$fund->receipt = !empty($keepReceipts) ? json_encode($keepReceipts) : null;
$fund->save();

</script>

<style>
.receipts-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 0.75rem; }
.receipt-wrapper { position: relative; }
.receipt-item { position: relative; width: 100px; height: 100px; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; background: #f9fafb; }
.receipt-item img { width: 100%; height: 100%; object-fit: cover; }
.file-link { display: flex; align-items: center; justify-content: center; text-align: center; font-size: 0.8rem; padding: 0.5rem; text-decoration: none; color: #4361ee; }
.checkbox-overlay {
    position: absolute;
    top: 5px; right: 5px;
...
.checkbox-overlay input { margin-right: 3px; }
