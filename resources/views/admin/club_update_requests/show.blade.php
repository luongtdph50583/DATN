                ...
                <div class="card mb-4 shadow-sm">
                    <div class="card-body">
                        ...
                        <table class="table table-bordered align-middle text-center">
                            ...
                            <tbody>
                                <tr>
                                    <td><strong>Giảng viên phụ trách</strong></td>
                                    {{-- Hiện tại --}}
                                    <td class="text-start">
        ...
        'treasurer' => 'Thủ quỹ'
            ...
            $clubManagerRecord = $currentData['members']->where('role', 'club_manager')->first();
                                            ...
                                            @if($clubManagerRecord)
                                                ...
                                                <div>Email: {{ $clubManagerRecord->member->user->email ?? '—' }}</div>
                                                <div>Ngày bổ nhiệm:
            ...
            $currentMembers = $currentData['members']->where('role', $role);
                                            ...
                                            @forelse($currentMembers as $cm)
                                                ...
                                                <div>Email: {{ $cm->member->user->email ?? '—' }}</div>
                                    ...
                                    <td>
                                        @php
        $proposedMembers = $proposedData['members']->where('role', $role);
                                        @endphp

                                        @forelse($proposedMembers as $pm)
                                            <div><strong>{{ $pm->user->name ?? 'Không rõ' }}</strong></div>
                                            <div>Email: {{ $pm->user->email ?? '—' }}</div>
